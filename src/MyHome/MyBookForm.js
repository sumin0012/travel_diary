...
export default function MyBookForm({ uid }) {

    const [title, setTitle] = useState("");
    const [text, setText] = useState("");
    const { addDocument, response } = useFirestore('Book');
    ...
    return (
        <>
            <form onSubmit={handleSubmit}>
                <fieldset>
                    <legend>나의 가계부</legend>
                    <label htmlFor="tit">가계부 제목 : </label>
                    <input id="tit" type='text' value={title} required onChange={handleData} />

                    <label htmlFor="txt">가계부 내용 : </label>
