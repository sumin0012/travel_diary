...
export const useCollection1 = (transaction, myQuery) => {

    // 문서들의 데이터를 관리합니다.
    const [documents1, setDocuments] = useState(null);
    // 에러 상태를 관리합니다.
    ...
    useEffect(() => {
        ...
        if (myQuery) {
            ...
            q = query(collection(appFireStore, transaction), where(...myQuery), orderBy("createdTime", "asc"));
            // 내림차순은 desc
        }
